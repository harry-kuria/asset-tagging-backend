...
const checkTrialStatus = (req, res, next) => {
  const { trialStartDate, isLicenseActive } = req.user;
  ...
  const trialEndDate = dayjs(trialStartDate).add(trialPeriodDays, 'days');
  const currentDate = dayjs();

  if (currentDate.isAfter(trialEndDate)) {
    return res.status(403).json({ message: 'Trial period has expired. Please purchase a license.' });
  }

  next(); // Trial is still valid, proceed to the next middleware
};

// User login route (authenticates the user)
app.post('/api/login', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE username = ?', [username], (err, userResult) => {
    ...
    if (userResult.length > 0) {
      ...
      const hashedPassword = user.password; // The hashed password from the DB
      ...
      bcrypt.compare(password, hashedPassword, (err, isMatch) => {
        ...
        if (isMatch) {
          console.log('Password match, fetching roles...');

          // Fetch user roles from the 'user_roles' table
          db.query('SELECT * FROM user_roles WHERE user_id = ?', [userId], (err, rolesResult) => {
            if (err) {
              console.error('Error fetching user roles: ', err);
              return res.status(500).json({ error: 'Internal Server Error' });
            }

            // If no roles are found, insert default roles for the user
            if (rolesResult.length === 0) {
              const defaultRoles = {
                userManagement: true,
                assetManagement: true,
                encodeAssets: true,
                addMultipleAssets: true,
                viewReports: true,
                printReports: true,
              };

              // Insert the default roles into the 'user_roles' table
              db.query(
                'INSERT INTO user_roles (user_id, userManagement, assetManagement, encodeAssets, addMultipleAssets, viewReports, printReports) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [
                  userId,
                  defaultRoles.userManagement,
                  defaultRoles.assetManagement,
                  defaultRoles.encodeAssets,
                  defaultRoles.addMultipleAssets,
                  defaultRoles.viewReports,
                  defaultRoles.printReports,
                ],
                (err, insertResult) => {
                  if (err) {
                    console.error('Error inserting default user roles: ', err);
                    return res.status(500).json({ error: 'Internal Server Error' });
                  }

                  console.log('Default roles inserted for new user.');
                  res.status(200).json({
                    success: true,
                    roles: defaultRoles,
                    trialStartDate,
              ...
              // Extract existing roles if found
              const userRoles = {
                userManagement: rolesResult[0].userManagement || false,
                assetManagement: rolesResult[0].assetManagement || false,
                encodeAssets: rolesResult[0].encodeAssets || false,
                addMultipleAssets: rolesResult[0].addMultipleAssets || false,
                viewReports: rolesResult[0].viewReports || false,
                printReports: rolesResult[0].printReports || false,
              };

              // Send the response with user roles and trial/activation status
              res.status(200).json({ success: true, roles: userRoles, trialStartDate, isLicenseActive });
...
//////////////////////ISSUE WITH CANVAS MODULE FOR WINDOWS///////////////////
...
app.post('/api/generate_invoice', checkTrialStatus,(req, res) => {
...
app.post('/api/addUser',checkTrialStatus, (req, res) => {
...
app.post('/api/create-account', async (req, res) => {
  ...
  try {
    ...
    db.query('SELECT * FROM users WHERE username = ?', [username], async (error, results) => {
      ...
      // Create a new user with default values for trialStartDate, licenseKey, and isLicenseActive
      db.query(
        'INSERT INTO users (username, password, trialStartDate, licenseKey, isLicenseActive) VALUES (?, ?, NULL, NULL, FALSE)',
...
app.get('/api/users/:id', checkTrialStatus,(req, res) => {
...
// Update a user by ID
app.put('/api/users/:id',checkTrialStatus, (req, res) => {
  const userId = req.params.id;
  const { username, password } = req.body;
  db.query(
...
// Delete a user by ID
app.delete('/api/users/:id',checkTrialStatus, (req, res) => {
  const userId = req.params.id;
...
app.get('/api/assets', checkTrialStatus,(req, res) => {
...
app.get('/api/institutions', checkTrialStatus,(req, res) => {
...
app.get('/api/departments',checkTrialStatus, (req, res) => {
...
app.get('/api/functionalAreas', checkTrialStatus,(req, res) => {
...
app.post('/api/addMultipleAssets/:assetType',checkTrialStatus, (req, res) => {
...
// Edit an asset
app.put('/api/assets/:id', checkTrialStatus,(req, res) => {
...
// Delete an asset
app.delete('/api/assets/:id', checkTrialStatus,(req, res) => {
  const assetId = req.params.id;
...
app.get('/api/categories', checkTrialStatus,(req, res) => {
...
app.get('/api/manufacturers',checkTrialStatus, (req, res) => {
...
app.post('/api/addAsset', checkTrialStatus,upload.single('logo'), (req, res) => {
  const {
    assetName,
    assetType,
    serialNumber,
...
app.get('/api/searchAssets',checkTrialStatus, (req, res) => {
...
app.get('/api/getAssetDetails',checkTrialStatus, (req, res) => {
...
app.post('/api/generateBarcodes',checkTrialStatus, async (req, res) => {
...
app.post('/api/generateBarcodesByInstitution',checkTrialStatus, async (req, res) => {
...
app.post('/api/fetchAssetsByInstitution',checkTrialStatus, async (req, res) => {
...
app.post('/api/generateBarcodesByInstitutionAndDepartment',checkTrialStatus, async (req, res) => {
...
app.get('/api/generateReport',checkTrialStatus, async (req, res) => {
  const {
    assetType,
    location,
    status,
